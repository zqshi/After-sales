...
> 接口前缀说明：业务/IM 接口统一前缀为 `/api/v1/api`；质量接口前缀为 `/api/v1`。文内若未写前缀，默认省略该前缀。
...
- **会话关闭（CloseConversation）**：IM对话永久存在，不支持关闭操作。这是IM的本质特性，而非设计选择。
  ...
  - MCP工具 `closeConversation` 已从 MCP 注册清单移除（避免误用关闭逻辑）
...
> 说明：以下接口均在 `imRoutes.ts` 中注册，完整前缀为 `/api/v1/api`（除质量接口外，质量接口前缀为 `/api/v1`）。

### 1) IM 会话与消息

- `GET /api/v1/api/im/conversations`：查询会话列表
- `GET /api/v1/api/im/conversations/stats`：会话统计
- `GET /api/v1/api/im/conversations/:id/messages`：拉取会话消息
- `POST /api/v1/api/im/conversations/:id/messages`：发送消息（人工/系统）
- `GET /api/v1/api/im/conversations/:id/problems`：会话问题列表
- `PATCH /api/v1/api/im/conversations/:id/status`：更新会话状态（IM 不允许 closed）
- `PATCH /api/v1/api/im/conversations/:id/mode`：设置会话模式

### 2) IM 测试/联调

- `POST /api/v1/api/im/wecom/mock/sync`：同步企微 mock 群聊数据

### 3) AI 与情绪分析

- `GET /api/v1/api/im/conversations/:id/ai-analysis`：AI 分析结果
- `GET /api/v1/api/im/conversations/:id/sentiment`：情绪分析结果

### 4) 质检接口（质量前缀）

- `GET /api/v1/quality/:conversationId`：会话质检概览
- `GET /api/v1/quality/:conversationId/reports`：会话质检报告列表
- `GET /api/v1/quality/reports`：质检报告查询

### 5) 客户画像接口

- `GET /api/v1/api/profiles/:customerId`：客户画像
- `GET /api/v1/api/profiles/:customerId/interactions`：客户互动记录

---

## 权限要求（与路由权限一致）

> 以下为最小权限需求（`config.permissions`），缺少权限将返回 403。
...
- MCP Server 在后端启动时默认注册（`AgentScopeGateway.initialize()`），无开关；是否调用取决于 AgentScope 侧编排。
- **✅ MCP 工具可用性已补齐**：已增加工具调用日志与E2E冒烟测试，用于验证核心工具可用性与稳定性。

**能力对照表（中文）**

| Agent | 已实现能力（默认可用） | 规划能力（代码存在/可用，需在Workflow/Agent侧编排启用） |
|---|---|---|
| Orchestrator Agent | 规则/启发式路由决策；执行模式选择；人工介入触发；可调用 MCP 工具（`analyzeConversation` / `getCustomerProfile` / `searchKnowledge`） | 规划能力主要在 AgentScope 侧（如意图/路由/升级人工），后端仅提供可被调用的 MCP 工具 |
| AssistantAgent | 情绪/意图分析、知识推荐、回复建议（AgentScope 侧完成；本仓库接收结果） | 推理型能力由 AgentScope 模型侧完成，不单独暴露 MCP 工具 |
| EngineerAgent | 知识检索、工单创建（`searchKnowledge` / `createTask` / `searchTickets` / `getSystemStatus`） | 复杂诊断/方案生成由 AgentScope 模型侧完成，不单独暴露 MCP 工具 |
| InspectorAgent | 质检评分、改进建议输出（AgentScope 侧生成），并通过 `saveQualityReport` / `createSurvey` 落库 | 质检报告生成由 `inspectConversation` / `generateQualityReport` 支撑 |

**AgentScope MCP 工具清单（本仓库提供给 AgentScope，基于当前 Agent 实际调用做了精简）**
...
- Workflow 定义：`workflows/*.yaml` 已存在，但默认不启用（避免影响主链路）

**默认启用状态（关键配置与影响）**
- Workflow 规划能力默认关闭：Workflow 仅保留为可选能力，避免影响主链路稳定性。如需启用，需显式打开配置并接通主链路。
- Workflow 动作能力（保留但未使用）：内置动作已覆盖意图识别、上下文获取、升级人工、故障诊断、质检等（详见 `ActionStepExecutor`），可按需扩展自定义动作。
- MCP Server 在后端启动时默认注册（无显式开关）；是否被 AgentScope 调用取决于 AgentScope 侧编排与提示词/策略。
...
| 工具 | 必填参数 | 关键说明 |
|---|---|---|
| getConversationHistory | conversationId | includeMetadata/limit 可选 |
| getCustomerProfile | customerId | 读取画像 |
| getCustomerHistory | customerId | limit/page 可选 |
| searchKnowledge | query | mode/filters 可选 |
| createTask | title | conversationId/requirementId 可选 |
| searchTickets | query | limit/offset 可选 |
| analyzeConversation | conversationId | includeHistory 可选 |
| inspectConversation | conversationId | 质检分析（含历史） |
| generateQualityReport | conversationId | 不落库 |
| saveQualityReport | conversationId, report | qualityScore/problemId 可选 |
| createSurvey | customerId, questions | conversationId/metadata 可选 |
| getSystemStatus |  | includeStats 可选 |

---

**MCP 可用性检查清单（建议冒烟项）**
- `GET /mcp/tools` 返回工具列表
- `tools/call: getSystemStatus` 返回 status 与 components
- `tools/call: searchKnowledge` 返回数组
- `tools/call: getConversationHistory` 返回历史消息列表
...
| 场景 | HTTP | 说明 |
|---|---|---|
| 缺少 method | 200 | 返回工具列表（兼容探测请求） |
| method 非法 | 400 | `unsupported method` |
| tools/call 缺少 name | 400 | `tool name is required` |
| 工具不存在 | 404 | `tool {name} not found` |
| 工具执行异常 | 500 | `tool execution failed` 或错误消息 |

---

**AgentScope 网关自检接口**

- `GET /agentscope/status`：返回服务状态与配置版本
- `GET /agentscope/config`：返回当前 AgentScope 配置
- `GET /agentscope/health`：返回健康状态与熔断器统计

**工具缺口清单**
- MCP 工具清单以本文件为准；是否实际被调用取决于 AgentScope 侧编排与运行策略
...
1. **Orchestrator Agent（路由编排）**
   ...
   - 规划能力备注（中文说明）：`classifyIntent/getConversationContext/escalateToHuman` 在 Workflow 执行器中有动作定义，但 Workflow 未接入主链路，且相关 MCP 工具未注册。
...
2. **AssistantAgent（对话辅助）**
   ...
   - 规划能力备注（中文说明）：`extractRequirement/assessRisk/recommendNextAction` 不再注册为 MCP 工具，由 AgentScope 模型侧推理完成。
...
3. **EngineerAgent（故障诊断）**
   ...
   - 规划能力备注（中文说明）：`classifyIssue/analyzeLogs/recommendSolution/estimateResolutionTime/createTechnicalTicket` 不再注册为 MCP 工具，由 AgentScope 模型侧推理完成；`getSystemStatus` 保留供健康预检使用。
...
4. **InspectorAgent（质检评分）**
   ...
   - 规划能力备注（中文说明）：`checkCompliance/detectViolations/compareTeamPerformance` 不再注册为 MCP 工具，由 AgentScope 模型侧推理完成。

**完备性检查**
- MCP 工具已注册；是否生效取决于 AgentScope 侧编排调用。Workflow 未接入主链路。
...
# WebSocket 联动（未接入）与 IM 回执（🚫 已废弃）

**✅ 实现状态**
- **WebSocket 推送（未接入）**：`backend/src/infrastructure/websocket/WebSocketService.ts` 存在，但未在 `app` 中注册，当前不可用。
- **当前功能**：人工复核通知仅通过 SSE 订阅（`/im/reviews/stream`）推送，未接入 WebSocket。
- **IM 回执接口（已废弃）**：`/api/im/messages/receipt` 接口已注释，原因：仅记录数据无业务逻辑（无失败告警、已读统计、超时提醒等），IM 渠道的消息送达状态追踪价值有限。

**规划说明**
- WebSocket 推送未接入，需实现服务注册与前端订阅
...
**说明**
- 当前仓库已创建 ReviewRequest（由 `ConversationTaskCoordinator` 触发），并可通过 `/im/reviews/submit` 回写以继续流程。
...
### 4. WebSocket 推送未接入
**问题描述**：`WebSocketService` 存在，但未在 `app` 中注册，实际链路不可用。
...
| 🟡 待处理 | WebSocket 推送未接入 | 实时性较差 | 补充 WebSocket 服务注册与前端订阅 |
