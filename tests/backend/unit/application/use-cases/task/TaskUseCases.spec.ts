...
describe('Task Use Cases', () => {
  ...
  describe('CreateTaskUseCase', () => {
    ...
    it('应该支持关联需求ID', async () => {
      ...
      expect(result).toBeDefined();
      expect(result.requirementId).toBe('00000000-0000-0000-0000-000000000002');
    });
  });
  ...
  describe('ListTasksUseCase', () => {
    ...
    it('应该成功列出所有任务', async () => {
      ...
      mockTaskRepo.countByFilters.mockResolvedValue(mockTasks.length);

      const result = await useCase.execute({});

      expect(result).toBeDefined();
      expect(result.items.length).toBe(2);
      expect(result.items[0].title).toBe('任务1');
      expect(result.items[1].title).toBe('任务2');
    });
    ...
    it('应该在没有任务时返回空数组', async () => {
      ...
      mockTaskRepo.countByFilters.mockResolvedValue(0);
