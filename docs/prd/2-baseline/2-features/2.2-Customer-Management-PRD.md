...
**功能描述**: 展示历史沟通记录并支持定位到当前对话内容；每个对话区域按问题聚合展示。

**展示内容**:
| 字段 | 说明 |
|------|------|
| title | 对话标题/摘要（无标题时显示首条消息摘要） |
| issueDescription | 问题描述（从需求/工单摘要中提取） |
| relatedProduct | 关联产品（产品名称或ID） |
| channel | 渠道（chat/email/phone/web/sms/voice） |
| status | 对话状态（open/pending/closed） |
| updatedAt | 最近更新时间 |
| lastMessagePreview | 最新一条消息摘要（最长50字） |
| unreadCount | 未读消息数量（若适用） |
| tags | 标签（如 VIP/高优先级） |

**外部依赖与降级展示**:
- **对话服务**: 若获取失败，显示“历史记录加载失败”并自动重试。
- **消息定位**: 若定位失败，提示“定位异常，请稍后重试”。
- **问题聚合**: 若无法关联问题/需求，显示“问题描述缺失”与空产品占位。
**交互行为**:
1. 时间范围下拉筛选
2. 点击“定位对话” → 关闭右侧面板并滚动定位
3. 点击“关联服务” → 打开服务记录详情弹窗

**边界条件**:
- 超过权限范围 → 处理方式详见[权限与入口管理说明](2.10-Permission-Management-PRD.md)
- 时间线为空 → 显示空状态
- 定位失败 → 展示提示并保持当前视图
...
**生成规则**:
1. **数据来源**: 最近30天对话消息、质检标签、工单/需求处理记录、满意度分数。
2. **情绪判断**: 以最近7天消息为主，负向情绪触发“风险”标签。
3. **风险等级**: 结合未解决问题数、超时次数与负向情绪，输出低/中/高。
4. **建议生成**: 基于相似历史案例与当前问题类型生成3条内建议。
5. **更新频率**: 每日定时自动刷新，遇到重大事件（投诉/退款）触发即时刷新。

**外部依赖与降级展示**:
- **质检/AI摘要**: 若不可用，显示“待生成”，支持定时自动刷新。

**交互行为**:
1. 默认直接展示摘要详情，当前不支持展开/编辑等交互
...
| 功能项 | 当前实现情况 | 说明 |
|-------|--------------|------|
| 客户信息卡片 | 已实现（后端接口） | 支持客户基础信息、客户等级、指标、风险等级、健康度 |
| 合同与指标 | 部分实现（指标） | 指标字段包含满意度、问题数、平均解决时长 |
| 已购产品 | 未实现 | 现有模型未包含产品/订阅字段 |
| 客情摘要 | 部分实现（insights） | 支持洞察条目（title/detail/source） |
| 历史对话时间线 | 部分实现（列表查询） | 通过对话列表按 customerId 查询 |
| 群信息 | 未实现 | 未见群公告/群备注相关接口 |
| 外部依赖展示 | 未实现 | 需前端/接口侧降级处理 |
...
**适用场景**: 仅有IM平台用户/群ID，尚未对接CRM客户主键。

**规范**:
- 单聊：`<channel>:user:<externalUserId>`  
- 群聊：`<channel>:group:<externalGroupId>`
- 允许匿名/临时：`<channel>:anon:<sessionId>`
...
**映射策略**:
- 若未来接入CRM，可通过映射表将多个IM身份合并到同一个CRM客户主键。
- 未匹配CRM时，`customerId` 仍可用于对话、质检、需求等流程，但客户画像可能为空。
...
```json
{
  ...
  "data": {
    ...
    "slaInfo": {
      ...
      "lastReviewedAt": "2026-01-12T08:00:00.000Z"
    },
    "metrics": {
      "satisfactionScore": 4.6,
      "issueCount": 12,
      "averageResolutionMinutes": 90,
      "lastUpdated": "2026-01-14T08:00:00.000Z"
    },
    "insights": [],
    "interactions": [],
    "serviceRecords": [],
...
**说明**: 创建客户画像，确保 `customerId` 唯一。可绑定外部CRM/画像系统身份。

**请求体**（示例）:
```json
{
  "customerId": "crm:customer:10001",
  "name": "张三",
  "contactInfo": {
...
**接口路径**: `/api/customers/{id}/bind-external`

**请求方法**: POST

**说明**: 将外部CRM/画像系统身份绑定到已有客户。
...
**接口路径**: `/api/customers/{id}/bindings`
...
| status | string | 否 | open/pending/closed | - |
| page | number | 否 | 页码 | 1 |
| limit | number | 否 | 每页 | 20 |

---

### 5.3 刷新客户画像

**接口路径**: `/api/customers/{id}/refresh`

**请求方法**: POST

**说明**: 供定时自动刷新触发，用于更新指标/洞察/互动/服务记录。
...
```json
{
  ...
  "metrics": {
    ...
    "issueCount": 10,
    "averageResolutionMinutes": 80
  },
  "insights": [
    { "title": "情绪稳定", "detail": "近期沟通情绪平稳", "source": "AI" }
  ]
}
```
...
**接口路径**: `/api/customers/{id}/interactions`
...
interface CustomerProfileView {
  ...
  insights: { title: string; detail: string; source?: string; createdAt?: string }[];
  interactions: { type: string; occurredAt: string; notes?: string; channel?: string }[];
  serviceRecords: { title: string; description: string; recordedAt?: string; ownerId?: string; outcome?: string }[];
  commitments: { id: string; title: string; progress: number }[];
  isVIP: boolean;
...
**说明**: 客户群与客户的关联需要明确字段映射，确保群信息与客户画像可关联查询。

| 客户群字段 | 客户字段 | 必填 | 说明 |
|-----------|----------|------|------|
| groupId | customerGroupId | 是 | 群唯一标识关联客户归属 |
| externalGroupId | externalGroupId | 否 | 跨系统的群ID映射（如IM渠道） |
| tenantId | tenantId | 是 | 多租户隔离字段需保持一致 |
