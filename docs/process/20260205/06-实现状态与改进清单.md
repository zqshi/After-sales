...
**已完成**
- 前端工单创建字段与 `docs/工单/工单相关接口.md` 对齐（分组布局 + 自动填充）
- 工单列表/详情展示字段对齐字典（状态/优先级/子类型映射）

**待补**
- 外部工单接口对接签名规则（`AppId/Timestamp/Sign`）
- workorder 代理默认 `WORKORDER_MODE=mock`，真实对接需启用 `proxy` 并配置 `WORKORDER_BASE_URL/APP_ID/APP_SECRET`
