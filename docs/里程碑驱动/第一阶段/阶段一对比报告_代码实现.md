...
**相关文档**
- 阶段一领域范围与架构/时序图：[阶段一_领域范围与架构图.md](./阶段一_领域范围与架构图.md)
...
#### 2.2.2 AI Agent
- 实际实现的 Agent：OrchestratorAgent、AssistantAgent、EngineerAgent、InspectorAgent、HumanAgent（HumanAgentAdapter）
- 对应能力：回复建议、情绪/意图、故障判断/解决方案、质检评分、人审协同

**调用图（Agent 协作架构）**
```mermaid
flowchart LR
  UI["Chat UI"] --> IM["IM Controller<br/>/im/*"]
  IM --> CTC["ConversationTaskCoordinator"]
  CTC --> ASG["AgentScopeGateway/ChatClient"]
  ASG --> ORCH["OrchestratorAgent"]
  ORCH --> AA["AssistantAgent"]
  ORCH --> EA["EngineerAgent"]
  ORCH --> HA["HumanAgentAdapter"]
  ORCH --> IA["InspectorAgent"]

  AA --> AI["LLM/External AI"]
  EA --> AI
  IA --> AI
  HA --> HUMAN["Human Review"]
```
...
sequenceDiagram
  ...
  participant CTC as ConversationTaskCoordinator
  participant ASG as AgentScopeGateway
  participant ORCH as OrchestratorAgent
  participant AA as AssistantAgent
  participant EA as EngineerAgent
  participant HA as HumanAgentAdapter

  UI->>IM: POST /im/incoming-message
  IM->>CTC: processCustomerMessage()
  CTC->>ASG: sendMessage(conversation)
  ASG->>ORCH: route(conversation)
  ORCH->>AA: analyze intent/sentiment
  ORCH->>EA: diagnose + solution
  alt supervised/human_first
    ORCH->>HA: request human review
    HA-->>ORCH: human feedback
  end
  ORCH-->>ASG: suggestion + metadata
  ASG-->>CTC: agentSuggestion
  CTC-->>IM: response
  IM-->>UI: suggestion + status
```

**代码证据与判断**
- Agent 列表与路由：`agentscope-service/src/api/routes/agents.py`、`agentscope-service/src/router/orchestrator_agent.py`
- Assistant/Engineer/Inspector/Human：`agentscope-service/src/agents/*_agent.py`
...
**结论**
- 主要能力链路存在，实际以 Orchestrator/Assistant/Engineer/Inspector/Human 协作实现“基础AI辅助”与质量管控/问题状态
...
2) AI Agent辅助功能
- 主要AI能力链路存在（Orchestrator/Assistant/Engineer/Inspector/Human + 回复建议/情绪/需求/任务/质检/问题状态） → 基本实现
