...
- 任务关联查询（Controller 侧）。
- 自动回复建议生成与人工复核触发（日志/后续可扩展 WebSocket 或事件）。
...
# IM 回执 / WebSocket 联动（规划）

**说明**
- 当前代码以日志/预留 TODO 为主，图示为规划链路。
- 若接入 IM 回执（送达/已读）或售后工程师实时通知，可用此图扩展实现。

**中文备注**
- 该图为“后续扩展方向”，不影响当前入站主流程。
- 适用于构建“复核请求推送/回执状态同步”的实时链路。
...
sequenceDiagram
  ...
  CTC-->>WS: emit("agent:review_request", suggestion) (TODO)
  WS-->>FE: 推送复核任务
  FE->>API: 人工审核/确认创建任务
  API-->>FE: 更新会话状态/任务列表
...
3. **AgentScope 质检**
   - 入口：问题解决事件处理器（ProblemResolvedHandler）
...
**说明**
- 当前以日志为主，预留 WebSocket/事件推送。
- 后续可扩展为：复核面板 → 生成任务/更新会话状态。

**中文备注**
- 该链路为“人机协同关键点”，建议后续在 WebSocket/事件层统一治理与监控。
- 复核动作最终应回写任务状态与建议处理结果。
